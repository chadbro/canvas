...
CSV.foreach(csv_file, {headers: true}) do |row|
	if row.headers[0] != 'canvas_course_id' || row.headers[1] != 'grading_id'
		puts "First column needs to be 'canvas_course_id', and second row needs to be 'grading_id'."
	else
	...
	get_course.on_complete do |response|
		if response.success?
			update_course = Typhoeus::Request.new(base_url + "api/v1/courses/#{row['canvas_course_id']}",
												  method: :put,
												  headers: default_headers,
												  params: { 'course[grading_standard_id]' => row['grading_id']})
